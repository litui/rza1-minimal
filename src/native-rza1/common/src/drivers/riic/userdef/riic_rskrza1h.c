...
******************************************************************************/
extern volatile struct st_riic * RIIC_GetRegAddr (uint32_t channel);
...
void r_riic_init_ch3 (void)
{
    ...
    /* ---- START Port Expander 1 & 2 Initial Output Setup ---- */
       ...
       Sample_RIIC_PortExpAccess(DEVDRV_CH_3, RZA1H_RSK_I2C_PX_IO1,
        ...
        /* Indicates which I/O is to be modified */
        iic_dev_io  = (uint8_t)(0xFF);  /* Modifying all bits of the register */
        iic_w_value = (uint8_t)(0x19);  /* Set bits as per the table below */
        ...
        IO[3] = PX1_EN3 Enable  = 1 (SSIF/PWM signal select.)
                                          ...
                                          uint32_t r_byte, uint8_t * r_buffer)
{
    ...
    R_RIIC_SendCond(RIIC_CH_3, RIIC_TX_MODE_START);
    ...
    {
    	R_RIIC_SendCond(RIIC_CH_3, RIIC_TX_MODE_RESTART);
        ...
        if (DEVDRV_SUCCESS == ret)
        {
            ...
            R_RIIC_SendCond(RIIC_CH_3, RIIC_TX_MODE_STOP);
        }
        else
        {
        	R_RIIC_SendCond(RIIC_CH_3, RIIC_TX_MODE_STOP);
