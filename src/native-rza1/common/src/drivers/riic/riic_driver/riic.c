...
*              : Disables the transmit-end interrupt. After checking the NACK 
*              : receive flag, waits until the conditions for the transmit data 
*              : empty notification information area satisfied by the user-defined 
*              : function Userdef_RIICn_WaitTransmitEmpty. After initializing 
*              : the information by using the user-defined function 
*              : Userdef_RIICn_InitTransmitEmpty (n=0 to 3), transmits the data. 
*              : This transmit operation continues until the specified byte counts 
*              : are reached.
*              : The data transmission is suspended when the NACK is received, 
*              : and DEVDRV_ERROR_RIIC_NACK is returned.
*              : After writing the data for the specified byte count to the I2C bus 
*              : transmit data register (RIICnDRT), enables the transmit end 
*              : interrupt. Waits until the conditions for the transmit end 
*              : notification information are satisfied by the user-defined function 
...
int32_t R_RIIC_Write(uint32_t channel, const uint8_t * buffer, uint32_t byte)
{
    ...
    riic = RIIC_GetRegAddr(channel);
    RZA_IO_RegWrite_8(&(riic->RIICnIER.UINT8[0]), 0, RIICn_RIICnIER_TEIE_SHIFT, RIICn_RIICnIER_TEIE);
    dummy_buf_8b = RZA_IO_RegRead_8(&(riic->RIICnIER.UINT8[0]), RIICn_RIICnIER_TEIE_SHIFT, RIICn_RIICnIER_TEIE);
#endif
    ...
    riic = RIIC_GetRegAddr(channel);
    RZA_IO_RegWrite_8(&(riic->RIICnIER.UINT8[0]), 1, RIICn_RIICnIER_TEIE_SHIFT, RIICn_RIICnIER_TEIE);
#endif 
...
*              : to the WAIT bit in the I2C bus mode register 3 (RIICnMR3) before 
*              : the I2C bus receive data register (RIICnDRR) is read. This holds 
*              : the period between the ninth clock and the first clock of the SCL 
*              : at the low level until the I2C bus receive data register (RIICnDRR) 
*              : is read when the last byte is received, so the state is such that 
*              : issuing a stop condition is possible. When a value 1 or 2 is 
*              : specified for the argument byte, 1 is set to the WAIT bit in 
*              : the I2C bus mode register 3 (RIICnMR3) before a dummy read of 
*              : the I2C bus receive data register (RIICnDRR), to output the SCL 
*              : clock, is performed.
...
int32_t R_RIIC_Read(uint32_t channel, uint8_t * buffer, uint32_t byte)
{
    ...
    if (1 == byte)
    {
        ...
        RIIC_Receive(channel, RIIC_RX_MODE_NACK, p_buffer2);

        /* Last byte read */
        RIIC_Receive(channel, RIIC_RX_MODE_STOP, p_buffer2);
    }
    else
    {
        /* Start reading data */
        ...
        if (2 == byte)
        {
            ...
            RIIC_Receive(channel, RIIC_RX_MODE_LOW_HOLD, p_buffer2);
        }
        else
        {
            /* normal start trigger */
            RIIC_Receive(channel, RIIC_RX_MODE_ACK, p_buffer2);
        }
